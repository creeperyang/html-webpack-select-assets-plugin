    ...
    describe(`HtmlWebpackSelectAssetsPlugin ${c.name} Mode`, function () {
        ...
        it('should do nothing when no patterns are specified', (done) => {
            webpack({
                ...
                expect(/script\s+.*?src\s*=\s*"(\/)?other(\.[a-z0-9]+\.min)?\.js"/i.test(html), 'could not find other bundle').to.be.true;
                expect(/script\s+.*?src\s*=\s*"(\/)?app(\.[a-z0-9]+\.min)?\.js"/i.test(html), 'could not find app bundle').to.be.true;
                expect(/link\s+.*?href\s*=\s*"(\/)?app(\.[a-z0-9]+\.min)?\.css"/i.test(html), 'could not find styles css bundle').to.be.true;
                done();
            });
        });
        ...
        it('should do nothing when no patterns & multi output pages generated', (done) => {
            webpack({
                ...
                expect(/script\s+.*?src\s*=\s*"(\/)?other(\.[a-z0-9]+\.min)?\.js"/i.test(html2), 'could not find other bundle').to.be.true;
                expect(/script\s+.*?src\s*=\s*"(\/)?app(\.[a-z0-9]+\.min)?\.js"/i.test(html2), 'could not find app bundle').to.be.true;
                expect(/link\s+.*?href\s*=\s*"(\/)?app(\.[a-z0-9]+\.min)?\.css"/i.test(html2), 'could not find styles css bundle').to.be.true;
                done();
            });
        });
                ...
                plugins: [
                    ...
                    new HtmlWebpackSelectAssetsPlugin({
                        selector: (asset, context, type) => {
                            expect(type).to.oneOf(['script', 'style', 'meta']);
                            const result = HtmlWebpackSelectAssetsPlugin.smartSelecor(asset, context, type);
                            if (type === 'script') {
                                expect(result).to.equal(context.entryFiles.includes(asset.attributes.src));
                            } else if (type === 'style') {
                                expect(result).to.equal(context.entryFiles.includes(asset.attributes.href));
                            } else {
        ...
        it('should only select correct assets if plugin.selector set to custom function', (done) => {
            webpack({
                ...
                expect(/link\s+.*?href\s*=\s*"(\/)?app(\.[a-z0-9]+\.min)?\.css"/i.test(html), 'could not find styles css bundle').to.be.true;
                const html2 = getOutput('other');
                expect(/script\s+.*?src\s*=\s*"(\/)?other(\.[a-z0-9]+\.min)?\.js"/i.test(html2), 'find other bundle').to.be.false;
                expect(/script\s+.*?src\s*=\s*"(\/)?other(\.[a-z0-9]+\.min)?\.css"/i.test(html2), 'find other bundle').to.be.false;
                done();
            });
        });
    });
